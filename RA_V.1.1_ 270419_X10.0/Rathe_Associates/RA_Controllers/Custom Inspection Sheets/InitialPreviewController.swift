//
//  InitialPreviewController.swift
//  Rathe_Associates
//
...
class InitialPreviewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate, UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIDocumentMenuDelegate, UIDocumentPickerDelegate, PECropViewControllerDelegate, UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == loadedData.count ||  indexPath.section == loadedData.count + 1
        {
            ...
            cell.hoBtn.constant = 44.0;
                ...
                cell.saveBtn.setTitle(translator("Close") , for: .normal)
        ...
        case "2":
            ...
            cell.innerView.backgroundColor = UIColor.lightGray.withAlphaComponent(0.2);
            addGrayBorders([cell.innerView]);
        ...
        case "3":
            ...
            else
            {
                if optTitles.count > 0
                {
                    ...
                    checkBoxViwe.firstBtn.setTitle(optTitles[0], for: .normal)
                    checkBoxViwe.secondBtn.isHidden = true;
                    checkBoxViwe.secondBtn.isEnabled = false;
                    //checkBoxViwe.firstBtn.addTarget(self, action: #selector(checkBoxTapped(_:)), for: .touchUpInside)
    ...
    func tableView(_ tableView: UITableView, estimatedHeightForRowAt indexPath: IndexPath) -> CGFloat {
        if  indexPath.section == loadedData.count ||  indexPath.section == loadedData.count + 1
        {
            if indexPath.section == loadedData.count
            {
                return 90
            }
    ...
    @objc func SaveAndCloseBtnTapped(_ sender : UIButton)
    {
        ...
        let alet = UIAlertController.init(title: "Alert", message: "Preview cannot be saved.", preferredStyle: .alert);
        alet.addAction(UIAlertAction.init(title: "Ok", style: .default, handler: nil));
            ...
            if GbuildIdentifier == builerIDE
            {
                ...
                for j in 0..<mymechData.count
                {
                    ...
                    if mechanicalRoomID == mechRoomId
                    {
                        ...
                        self.previewTable.isHidden = false;
    ...
    {
        headertitle.text = headertitleText! + " - Preview"
        ...
        Alamofire.request("\(inspectionFormAPI)/\(2)/\(1)").responseJSON { (responseData) in
