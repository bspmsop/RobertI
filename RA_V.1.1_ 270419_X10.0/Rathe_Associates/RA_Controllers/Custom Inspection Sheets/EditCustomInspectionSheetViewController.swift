//
//  EditCustomInspectionSheetViewController.swift
//  Rathe_Associates
//
...
class EditCustomInspectionSheetViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate, UITextViewDelegate  {
    ...
    var primaryData :  Dictionary<String, String> = ["company" : "", "mechanical_Room": "", "inspection_name": "", "inspection_interval" : "", "mechid" : "", "comid":"", "idate" : ""];
    ...
    @objc func getTheData(_ sender : UITextFeild)
    {
        if sender.texet.section == 0
        {
            ...
            case 1:
                ...
                {
                    generalPickerData = mechanicalListData;
                    samplePrimaryfield = sender;
                    menuDropDown.reloadAllComponents();
                }
                break;
            case 2:
                ...
                break;
            case 3:
                generalPickerData = JSON(intervalListData).arrayValue;
                samplePrimaryfield = sender;
                menuDropDown.reloadAllComponents();
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if mytext != nil
        {
            ...
            //            let optionsstr = textView.text;
            //            let optionarray = optionsstr?.split(separator: "\n");
            //            print(optionarray);
        ...
        if row < generalPickerData.count
        {
            ...
            if samplePrimaryfield.texet.section == 0
            {
                ...
                case 0:
                    ...
                    primaryData["mechid"] = ""
                    primaryData["mechanical_Room"] = "";
                    
                    DispatchQueue.main.async {
                        
                        self.mechanicalListData = self.generalPickerData[row]["mechs"].arrayValue
                ...
                case 1:
                    samplePrimaryfield.text = generalPickerData[row]["title"].stringValue
                    primaryData["mechanical_Room"] = generalPickerData[row]["title"].stringValue
                    primaryData["mechid"] = generalPickerData[row]["id"].stringValue
                    
                case 3:
                    if row == 5
                    {
                        samplePrimaryfield.text = ""
                        samplePrimaryfield.endEditing(true);
                        primaryData["inspection_interval"] = generalPickerData[row].stringValue
                        primaryData["idate"] = "";
                        popTabl.loadingDefaultUI();
                        self.popTabl.wariningLab.text = "";
                        self.popTabl.roomNameField.text  = "";
                        self.popTabl.headertitle.text = "Enter Inspection Interval(days)";
                        self.popTabl.roomNameField.keyboardType = .numberPad;
                        self.popTabl.roomNameField.placeholder = "days"
            ...
            else
            {
                ...
                else
                {
                    ...
                    self.sectionData[samplePrimaryfield.texet.section - 1]["fields"] = fdata;
                    self.inspectionTable.reloadRows(at: [samplePrimaryfield.texet], with: .fade)
    ...
    @objc func deleteSection(_ sender : UIBotton)
    {
        ...
        DispatchQueue.main.async {
            ...
            if  self.sdelete.isEmpty
            {
                
                let id = String(self.sectionData[sender.tag]["section_id"] as! Int)
                if id != "0"
                {
                    self.sdelete =  id
                }
            ...
            UIView.animate(withDuration: 0.5, animations: {
                ...
                self.inspectionTable.reloadData();
                self.inspectionTable.scrollToRow(at: IndexPath.init(row: 0, section: sender.tag + 1), at: .top, animated: false);
                self.view.endEditing(true);
    ...
    @objc func addSectionInTable(_ sender : UIBotton)
    {
        DispatchQueue.main.async {
            ...
            let sample : Dictionary<String, Any> =  ["section_id" : 0, "head" : "", "order" : orderstr, "cols" : "1", "fields": [["label":"", "order" : "1", "itype" : "1", "ioptions" : "", "required" : "N", "ivalids" : "2", "idefault" : "", "id" : 0]]]
        ...
        UIView.animate(withDuration: 0.5, animations: {
            ...
            DispatchQueue.main.async(execute: {
                self.inspectionTable.reloadData();
                self.inspectionTable.scrollToRow(at: IndexPath.init(row: 0, section: sender.tag + 2), at: .top, animated: false);
                self.view.endEditing(true);
            })
    ...
    @objc func addRowInTable(_ sender : UIBotton)
    {
        ...
        DispatchQueue.main.async {
            ...
            let mydata : Dictionary<String, Any> =   ["label" : "", "order" : rowOrder, "itype" : "1","typename": "Text box","ioptions" : "", "required" : "N", "ivalids" : "2", "idefault" : "", "id" :  0]
            fdata.append(mydata);
        ...
        UIView.animate(withDuration: 0.5, animations: {
            ...
            self.inspectionTable.reloadSections(IndexSet.init(integer: sender.setTitlein.section), with: .none);
            self.inspectionTable.scrollToRow(at: IndexPath.init(row: fdata.count - 1, section: sender.setTitlein.section), at: .none, animated: false);
    ...
    @objc func deleteRowInTable(_ sender : UIBotton)
    {
        ...
        DispatchQueue.main.async {
           ...
           if self.fdelete.isEmpty
           {
            let id =  String(fdata[sender.setTitlein.row]["id"] as! Int)
            if id != "0"
            {
                self.fdelete =  id
            }
    ...
    @objc func SaveAndCloseBtnTapped(_ sender : UIBotton)
    {
        ...
        if checkNetworks.connection == .none
        {
            DispatchQueue.main.async {
                ...
                alerts.addAction(UIAlertAction.init(title: "ok", style: .default, handler: { (_) in
                    ...
                    let savestatsu =   saetolocaldatabase(jdata, "addcustominspection");
                    if savestatsu
                    {
        ...
        let params = ["cinsdata" :  createJSOndata()] ;
        
        print(params);
        
        print(vAddCustomInspectionSaveapi);
            ...
            if resp.result.value != nil
            {
                ...
                if statuscode == 1
                {
                    ...
                    DispatchQueue.main.async {
                        ...
                        let alert = UIAlertController.init(title:  translator("Success!"), message: translator("Successfully updated inspection sheet"), preferredStyle: .alert);
    ...
    func createJSOndata() -> JSON
    {
        ...
        dictjs["mech_id"] = primaryData["mechid"]
        dictjs["uid"] =   cachem.string(forKey: "userid")!
        dictjs["ctime"] = primaryData["inspection_interval"]!;
        dictjs["idate"] = primaryData["idate"]!;
        
        dictjs["section_delete"] = sdelete
        dictjs["field_delete"] = fdelete
        ...
        for l in 0..<sectionData.count
        {
            ...
            setioonids.append(dicter["section_id"]!)
            
            
            setionSno.append(sid)
            sectionSort.append(dicter["order"]!)
            sectioninputrows.append(dicter["cols"]!)
            ...
            let fieldsdata = dicter["fields"] as! [Dictionary<String, Any>]
            ...
            for v in 0..<fieldsdata.count
            {
                ...
                {
                    fieldsectionids.append(dicter["section_id"]!);
                }
                
                fieldrequired.append(fdatar["required"]!);
    ...
    func checkvalidationData() -> Dictionary<String, String>
    {
        ...
        let comid = primaryData["comid"]!;
        let mechid = primaryData["mechid"]!;
        let insName = primaryData["inspection_name"]!;
        let insival = primaryData["inspection_interval"]!;
        ...
        else if mechid.isEmpty
        {
            validationdict["isvalid"] = "0"
            validationdict["emsg"] = "Please select mechanical room"
            
        }
        else if insName.isEmpty
        {
            validationdict["isvalid"] = "0"
            validationdict["emsg"] = "Please enter inspection title"
            
            
        }
        else if insival.isEmpty
        {
            validationdict["isvalid"] = "0"
            validationdict["emsg"] = "Please select inspection interval"
            ...
            for dicter in sectionData
            {
                ...
                if sectionhead.isEmpty
                {
                    ...
                    {
                        let subheader = fdatar["label"] as! String
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0
        {
            ...
            case 1:
                ...
                if comid.isEmpty
                {
                    cell.innerTextField.inputView = UIView();
                }
                else
                {
                    cell.innerTextField.inputView = menuDropDown;
                ...
                cell.innerTextField.addTarget(self, action: #selector(getTheData(_:)), for: .editingDidBegin);
                cell.imgWth.constant = 25;
                cell.titleLabel.text =    "Mechanical Room:"
                cell.innerTextField.text = primaryData["mechanical_Room"]
                
            case 2:
                cell.innerTextField.addTarget(self, action: #selector(getTheData(_:)), for: .editingChanged);
                cell.imgWth.constant = 0.0;
                cell.titleLabel.text =   "Inspection Form Name:"
            ...
            case 3:
                ...
                cell.innerTextField.addTarget(self, action: #selector(getTheData(_:)), for: .editingDidBegin);
                cell.imgWth.constant = 25;
                cell.titleLabel.text =  "Inspection Interval:"
                cell.innerTextField.text = primaryData["inspection_interval"]
                
                let interval = primaryData["inspection_interval"]!
                cell.innerTextField.text = primaryData["inspection_interval"]!
                let idate  = primaryData["idate"]!
                
                if interval  == "custom" && !idate.isEmpty
                {
                    cell.innerTextField.text = primaryData["idate"]
                }
        ...
        else if  indexPath.section == sectionData.count + 1
        {
            ...
            if indexPath.row == 0
            {
                ...
                cell.saveBtn.backgroundColor = UIColor.init(red: 32/255, green: 191/255, blue: 5/255, alpha: 1.0)
                 cell.saveBtn.setTitle("Update Custom Form", for: .normal)
            ...
            else if indexPath.row == 1
            {
                ...
                cell.topPadding.constant = 20
                cell.saveBtn.backgroundColor = UIColor.init(red: 32/255, green: 191/255, blue: 5/255, alpha: 1.0)
                cell.saveBtn.setTitle("Save As", for: .normal)
                cell.saveBtn.addTarget(self, action: #selector(saveAsTapped(_:)), for: .touchUpInside);
        ...
        else if  indexPath.section > 0  && indexPath.section < sectionData.count + 1
        {
            ...
            var trows = sectionData[indexPath.section - 1]["fields"] as! Array<Dictionary<String, Any>>
            if indexPath.row == 0
            {
                if trows.count < 2
                {
            ...
            for (key, value) in fieldTypeDataVlaue
            {
                if value == itypeer
                {
                    cell.roeType.text = key
                }
            ...
            let forder = trows[indexPath.row]["order"] as? String;
            cell.fieldOrderFd.text = trows[indexPath.row]["order"] as? String;
            if forder == nil
            {
                
                 cell.fieldOrderFd.text = "\(indexPath.row + 1)"
                trows[indexPath.row]["order"]  = "\(indexPath.row + 1)"
                sectionData[indexPath.section - 1]["fields"] = trows
            }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section > 0 && section < sectionData.count + 1
        {
            ...
            let ord = sectionData[section - 1]["order"] as? String
            headV.displayorderField.text = sectionData[section - 1]["order"] as? String
            if ord == nil
            {
                 headV.displayorderField.text =  "\(section)"
                sectionData[section - 1]["order"] = "\(section)"
            ...
            let ncolumns = sectionData[section - 1]["cols"] as! String
    ...
    @objc func ncolumnsTapped(_ sender : UIBotton)
    {
        ...
        sectionData[sender.hasTag]["cols"]  = String(sender.tag)
    ...
    @objc func isRequiredTapped(_ sender : UIBotton)
    {
        ...
        if sender.tag == 1
        {
            ...
            case "Y":
                ...
                fdata[sender.setTitlein.row]["required"]  = "N"
                self.sectionData[sender.setTitlein.section - 1]["fields"] = fdata;
        ...
        else
        {
            ...
            case "N" :
                ...
                fdata[sender.setTitlein.row]["required"]  = "Y"
                self.sectionData[sender.setTitlein.section - 1]["fields"] = fdata;
    ...
    func calldefaultData()
    {
        ...
        let Buildingapi = "\(vEditCustomInspectionDetailAPI)\(userid)/\(usertype)/\(sinspectioon_id)"
        print(Buildingapi);
        Alamofire.request(Buildingapi, method: .get).responseJSON { (resp) in
            ...
            if resp.result.value != nil
            {
                ...
                let scode = resultdata["sections"].arrayObject
                let temp = resultdata["template"]
                
                if  scode != nil && temp != JSON.null
                {
                    let testdata  = scode as? Array<Dictionary<String, Any>>;
                    
                    if testdata != nil
                    {
                        
                        DispatchQueue.main.async {
                            self.sectionData = testdata!
                            isOfflineMode = false
                            
                            self.companyListData = resultdata["data"].arrayValue
                            self.primaryData["company"] = temp["cname"].stringValue
                                 self.primaryData["mechanical_Room"] = temp["mtitle"].stringValue
                                 self.primaryData["comid"] = temp["code"].stringValue
                            self.primaryData["mechid"] = temp["mech_id"].stringValue
                            self.primaryData["inspection_name"] = temp["name"].stringValue
                             self.primaryData["inspection_interval"] = temp["ctime"].stringValue
                            self.inspectionName.text =  temp["name"].stringValue
                            
                            for l in 0..<self.companyListData.count
                            {
                                let cd = self.companyListData[l]["cid"].stringValue
                                if cd == temp["code"].stringValue
                                {
                                    self.mechanicalListData =  self.companyListData[l]["mechs"].arrayValue
                                    break;
                                }
    ...
    @objc func saveAsTapped(_ sender : UIBotton)
    {
        let vController = self.storyboard?.instantiateViewController(withIdentifier: "SaveAsPopupViewController") as! SaveAsPopupViewController
        vController.sinsid = sinspectioon_id;
