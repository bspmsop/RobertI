...
class AddEquipmentViewController:UIViewController, UITableViewDelegate, UITableViewDataSource, UIPickerViewDataSource, UIPickerViewDelegate,UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIDocumentMenuDelegate, UIDocumentPickerDelegate, PECropViewControllerDelegate  {
    ...
    var BuildDetils = ["Equiname" : "", "Model" : "",  "Serial" : "",  "Mechanicalroom" : "", "mid" : "", "testform" : "", "testid" : ""];
    var hud = MBProgressHUD();
    
    @IBOutlet weak var addEquipmentTable: UITableView!
    ...
    @objc func addAttachment(_ sender : UIBotton)
    {
        ...
        UIView.animate(withDuration: 0.05, animations: {
            ...
            self.addEquipmentTable.reloadData();
            print("reloaded data")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        else if  indexPath.section == 3
        {
            ...
            else
            {
                ...
                cell.saveBtn.setTitle("Create New Equipment", for: .normal)
    ...
    func uploadimagetoserver()
    {
        ...
        if imageneedtouploaddata.count > 0 && imageneedtouploaddata.count > imageuploadercount
        {
            ...
            }, to: vEquipmentUploadimageApi , method: .post ) { (resp) in
        ...
        {
            let cj = createJSON();
            let parms = ["eqpdata" : cj];
            if self.gotmechid == "-1"
            {
                
                AddCBMechanicalRoomViewController.newequipdata = cj
                self.navigationController?.popViewController(animated: true);
            ...
            Alamofire.request(vEquipmentCreatesaveApi, method: .post, parameters: parms).responseJSON { (resp) in
                ...
                if resp.result.value != nil && resp.result.isSuccess
                {
                    ...
                    DispatchQueue.main.async(execute: {
                        ...
                        let alert = UIAlertController.init(title:  translator("Success"), message: translator("Equipment created successfully"), preferredStyle: .alert);
        ...
        if checkNetworks.connection == .none
        {
            DispatchQueue.main.async {
                ...
                alerts.addAction(UIAlertAction.init(title: "ok", style: .default, handler: { (_) in
                    ...
                    let savestatsu =   saetolocaldatabase(jdata, "addequipment");
                    if savestatsu
                    {
    ...
    func getequipmentrelationData()
    {
        ...
        let Buildingapi = "\(vEquipmentCreatedataApi)\(userid)/\(usertype)"
        print(Buildingapi)
        Alamofire.request(Buildingapi, method: .get).responseJSON { (resp) in
            ...
            if resp.result.value != nil && resp.result.isSuccess
            {
                ...
                var resultdata =  JSON(resp.result.value!);
                    ...
                    isOfflineMode = false
                     let fdata = resultdata["mechs"]
                self.equipmenttestformlist = fdata["eqps"].arrayValue;
                 self.mechanciarlroomslist = fdata["mechs"].arrayValue;
                
                self.BuildDetils["Mechanicalroom"] = self.gotmechtitle
                    self.BuildDetils["mid"] =  self.gotmechid;
                
                 DispatchQueue.main.async {
                        self.addEquipmentTable.reloadData();
                        self.addEquipmentTable.isHidden = false;
                        self.hud.hide(animated: true);
                    }
    ...
    func loadingDefaultUI()
    {
        ...
        self.addEquipmentTable.isHidden = true;
